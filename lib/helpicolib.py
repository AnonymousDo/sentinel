...
def is_valid_helpico_address(address, network='mainnet'):
    ...
    helpico_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Helpico address
        return False

    if (address_version != helpico_version):
        return False
...
# shims 'til we can fix the helpicod side
def SHIM_serialise_for_helpicod(sentinel_hex):
    from models import HELPICOD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = HELPICOD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in helpicod
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # helpicod expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    helpicod_hex = serialise(obj)
    return helpicod_hex


# shims 'til we can fix the helpicod side
def SHIM_deserialise_from_helpicod(helpicod_hex):
    from models import HELPICOD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(helpicod_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('helpico.conf')
