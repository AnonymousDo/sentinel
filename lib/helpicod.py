...
class HelpicoDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some helpicod methods
    ...
    @classmethod
    def from_helpico_conf(self, helpico_dot_conf):
        from helpico_config import HelpicoConfig
        config_text = HelpicoConfig.slurp_config_file(helpico_dot_conf)
        creds = HelpicoConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = helpicolib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = helpicolib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = helpicolib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_helpicod_proto_version_with_sentinel_ping)
