...
def has_helpico_conf():
    import config
    import io

    valid_helpico_conf = False

    # ensure helpico_conf exists & readable
    #
    # if not, print a message stating that Helpico Core must be installed and
    # configured, including JSONRPC access in helpico.conf
    try:
        f = io.open(config.helpico_conf)
        valid_helpico_conf = True
    except IOError as e:
        print(e)

    return valid_helpico_conf
...
def main():
    ...
    if not has_helpico_conf():
        print("HelpicoCore must be installed and configured, including JSONRPC access in helpico.conf")
        sys.exit(1)
