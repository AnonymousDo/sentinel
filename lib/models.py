...
class GovernanceObject(BaseModel):
    ...
    # sync helpicod gobject list with our local relational DB backend
    @classmethod
    def sync(self, helpicod):
        golist = helpicod.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_helpicod(helpicod, item)
    ...
    @classmethod
    def import_gobject_from_helpicod(self, helpicod, rec):
        import decimal
        import helpicolib
        import inflection
        ...
        # shim/helpicod conversion
        object_hex = helpicolib.SHIM_deserialise_from_helpicod(object_hex)
        objects = helpicolib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from helpicod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from helpicod, with every run
        ...
        # get/create, then sync payment amounts, etc. from helpicod - Helpicod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(helpicod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from helpicod! %s" % e)
            govobj.vote_delete(helpicod)
            return (govobj, None)
    ...
    def vote_delete(self, helpicod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(helpicod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, helpicod, signal, outcome):
        import helpicolib
        ...
        output = helpicod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = helpicolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(helpicod, signal)

    def sync_network_vote(self, helpicod, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = helpicod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = HELPICOD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 helpico addr, non-multisig
            if not helpicolib.is_valid_helpico_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Helpico address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (helpicolib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/HelpicoDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import helpicolib
        obj_data = helpicolib.SHIM_serialise_for_helpicod(self.serialise())
    ...
    def prepare(self, helpicod):
        try:
            object_hash = helpicod.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = HELPICOD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not helpicolib.is_valid_helpico_address(addr, config.network):
    ...
    def hash(self):
        import helpicolib
        return helpicolib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = HELPICOD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, helpicod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - helpicod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, helpicod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - helpicod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, helpicod):
        now = int(time.time())
        return (self.created_at < (now - helpicod.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, helpicod):
        if self.is_expired(helpicod):
            return False

        return True

    def is_deletable(self, helpicod):
        if self.is_expired(helpicod):
            return True
